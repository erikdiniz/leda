...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private int heightRecursive (BSTNode<T> currentNode) {
		...
		if (currentNode.isEmpty())
			...
			height += 1 + Math.max(this.heightRecursive((BSTNode<T>) currentNode.getLeft()), this.heightRecursive((BSTNode<T>) currentNode.getRight()));
	...
	public BSTNode<T> search(T element) {
		...
		if (element != null)
			node = this.searchRecursive(this.root, element);
	...
	public BSTNode<T> maximum() {
		...
		if (!this.isEmpty())
			node = this.maximumRecursive(this.root);
